  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-slate-100 px-1 active:bg-brand/5 transition-colors">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            {isLow && (
              <div className="flex items-center gap-1 bg-rose-50 px-1.5 py-0.5 rounded-md">
                ...
                <span className="text-[8px] font-black text-rose-600 uppercase">Low Stock</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="text-right flex-shrink-0 ml-3">
        ...
        <p className="text-[9px] font-bold text-slate-300 uppercase tracking-tight mt-0.5">Available</p>
      </div>
    </div>
  );
};
...
export default function StockRegistryPage() {
  ...
  const lowStockCount = filteredStocks.filter(s => s.quantity < 5).length;
    ...
    <div className="p-6 pb-24 flex flex-col gap-5 min-h-screen bg-white">
      <header className="flex items-center justify-between pt-4">
        ...
        <button 
          ...
          className="h-10 w-10 border border-slate-200 rounded-xl flex items-center justify-center text-brand active:scale-90 transition-transform bg-white shadow-sm"
        >
      ...
      <section className="flex flex-col">
        <div className="flex items-center justify-between mb-3 px-1 border-b border-slate-100 pb-2">
          <h2 className="text-[10px] font-black text-text-secondary uppercase opacity-30">
            {loading ? 'Consulting Ledger...' : `${filteredStocks.length} Records • ${lowStockCount} Alerts`}
          </h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 opacity-20">
            ...
            <p className="text-[11px] font-bold mt-4">No stock records found</p>
          </div>
        )}
      </section>
    </div>
