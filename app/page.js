...
const StatCard = memo(({ title, value, trendValue, icon: Icon, isLoading, gradient, onClick }) => {
  if (isLoading) {
    return (
      <div className="bg-card p-3.5 rounded-[1.75rem] min-w-[160px] snap-center flex flex-col gap-3.5 overflow-hidden relative border border-glass-border/20 shadow-sm">
  ...
  return (
    <div 
      ...
      className={`bg-card p-3.5 rounded-[1.75rem] min-w-[160px] snap-center flex flex-col gap-3.5 transition-transform border border-glass-border/30 shadow-sm ${onClick ? 'active:scale-[0.97] cursor-pointer' : ''}`}
    >
      <div className="flex items-start justify-between">
...
const ActionCard = memo(({ title, description, icon: Icon, color, isLoading, onClick }) => {
  if (isLoading) {
    return (
      <div className="bg-card p-3 rounded-3xl flex items-center gap-4 relative overflow-hidden border border-glass-border/20 shadow-sm">
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full animate-[shimmer_1.5s_infinite]" />
  ...
  return (
    <button
      ...
      className="bg-card p-3 rounded-3xl flex items-center gap-4 text-left active:scale-[0.98] transition-all hover:bg-surface-muted/30 border border-glass-border/30 shadow-sm"
    >
...
export default function Home() {
  ...
  return (
    <div className="h-[100dvh] overflow-y-auto no-scrollbar px-4 pb-24 flex flex-col gap-8 pt-[var(--sat)] pb-[var(--sab)] animate-in fade-in duration-700">
      ...
      {statsError && (
        <div className="bg-card p-4 rounded-2xl flex items-center justify-between border border-rose-500/20 shadow-sm">
