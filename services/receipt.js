...
export const receiptService = {
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .save-bar { position: sticky; top: 0; background: #000000; padding: 11px 24px; display: flex; align-items: center; justify-content: space-between; z-index: 100; }
    .save-btn { background: #000000; color: #fff; font-family: system-ui, -apple-system, sans-serif; font-weight: 700; font-size: 13px; border: none; padding: 9px 22px; border-radius: 7px; cursor: pointer; letter-spacing: 0.3px; }
    .save-hint { color: #ffffff; font-size: 12px; font-weight: 500; }
    .wrap { padding: 16px 20px; }
    .label { font-size: 10px; font-weight: 700; color: #666666;  letter-spacing: 1.2px; }
    .value { font-size: 13px; font-weight: 500; color: #000000; margin-top: 2px; }
    @media print {
      @page { size: A4; margin: 6mm 8mm; }
  ...
  <div class="save-bar no-print">
    ...
    <div style="display:flex;align-items:center;gap:14px;">
      <span class="save-hint">Change destination to <strong style="color:#ffffff;">Save as PDF</strong></span>
  ...
  <div class="wrap">
    <!-- Header -->
    <div style="display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:28px;border-bottom:1.5px solid #cccccc;margin-bottom:28px;">
      <div>
        ${businessLogo ? `<img src="${businessLogo}" style="height:52px;object-fit:contain;margin-bottom:14px;display:block;" />` : ''}
        <div style="font-size:20px;font-weight:800;color:#000000;letter-spacing:-0.5px;">${businessName || 'Inzeedo'}</div>
        ${companyAddress ? `<div style="margin-top:6px;font-size:12.5px;color:#000000;font-weight:400;line-height:1.6;max-width:260px;">${companyAddress}</div>` : ''}
        ${companyPhone ? `<div style="margin-top:6px;font-size:12.5px;color:#000000;display:flex;align-items:center;gap:6px;">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
          ${companyPhone}
        </div>` : ''}
        ${companyEmail ? `<div style="margin-top:4px;font-size:12.5px;color:#000000;display:flex;align-items:center;gap:6px;">
        ...
        ${taxId ? `<div style="margin-top:4px;font-size:12.5px;color:#000000;display:flex;align-items:center;gap:6px;">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
          VAT/TIN: ${taxId}
        </div>` : ''}
      </div>
      <div style="text-align:right;">
        <div style="font-size:28px;font-weight:900;color:#000000;letter-spacing:-1.5px;">Invoice</div>
        <div style="margin-top:18px;display:flex;flex-direction:column;gap:12px;">
          <div>
            <div class="label">Invoice Number</div>
            <div style="font-size:15px;font-weight:700;color:#000000;margin-top:2px;">${sale.invoice_number || 'DRAFT'}</div>
          </div>
          <div style="display:flex;justify-content:flex-end;gap:6px;font-size:12px;font-weight:600;color:#000000;margin-top:-6px;">
            <span>${invoiceDate}</span>
            ${invoiceTime ? `<span style="color:#999999;">&bull;</span><span>${invoiceTime}</span>` : ''}
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Items Table -->
    <table style="width:100%;border-collapse:collapse;margin-bottom:28px;border:1px solid #cccccc;border-radius:10px;overflow:hidden;">
      <thead>
        <tr style="background:#ffffff;">
          <th style="padding:11px 14px;text-align:left;font-size:10px;font-weight:700;color:#000000;border-bottom:1.5px solid #cccccc;width:5%;">&#35;</th>
          <th style="padding:11px 14px;text-align:left;font-size:10px;font-weight:700;color:#000000;border-bottom:1.5px solid #cccccc;width:35%;">Item Description</th>
          <th style="padding:11px 14px;text-align:left;font-size:10px;font-weight:700;color:#000000;border-bottom:1.5px solid #cccccc;width:15%;">Pack / Size</th>
          <th style="padding:11px 14px;text-align:center;font-size:10px;font-weight:700;color:#000000;border-bottom:1.5px solid #cccccc;width:10%;">Qty</th>
          <th style="padding:11px 14px;text-align:right;font-size:10px;font-weight:700;color:#000000;border-bottom:1.5px solid #cccccc;width:17.5%;">Unit Price</th>
          <th style="padding:11px 14px;text-align:right;font-size:10px;font-weight:700;color:#000000;border-bottom:1.5px solid #cccccc;width:17.5%;">Amount</th>
        </tr>
      </thead>
      <tbody>
        ${itemRows}
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:28px;">
      <div style="text-align:center;flex-shrink:0;">
        <img src="${qrUrl}" style="width:100px;height:100px;border:1px solid #cccccc;padding:7px;border-radius:10px;display:block;" />
        <div style="font-size:9px;font-weight:600;color:#666666;margin-top:7px;max-width:120px;line-height:1.4;">Scan to verify authenticity</div>
      </div>
      <div style="flex:1;max-width:320px;border:1px solid #cccccc;border-radius:10px;padding:18px;font-family:'Inter',sans-serif;">
        <div style="display:flex;justify-content:space-between;margin-bottom:9px;">
          <span style="font-size:13px;color:#000000;font-weight:500;">Subtotal</span>
          <span style="font-size:13px;font-weight:600;color:#000000;">${parseFloat(sale.total_amount).toLocaleString(undefined,{minimumFractionDigits:2})}</span>
        </div>
        ${discountRow}${taxRow}${adjRow}
        <div style="display:flex;justify-content:space-between;font-size:19px;font-weight:800;color:#000000;border-top:1.5px dashed #999999;padding-top:13px;margin-top:10px;letter-spacing:-0.5px;">
          <span>Total</span>
          <span>${parseFloat(sale.payable_amount).toLocaleString(undefined,{minimumFractionDigits:2})}</span>
        </div>
        ${!isManufacturing ? `
        <div style="margin-top:14px;background:#ffffff;padding:13px;border-radius:8px;">
          <div style="display:flex;justify-content:space-between;">
            <span style="font-size:12px;font-weight:500;color:#000000;">Paid via ${(sale.payments?.[0]?.payment_method || sale.payment_method || 'Cash')}</span>
            <span style="font-size:12px;font-weight:700;color:#000000;">${parseFloat(sale.paid_amount || sale.payable_amount).toLocaleString(undefined,{minimumFractionDigits:2})}</span>
          </div>
          ${changeRow}
        </div>
        ` : (sale.paid_amount > 0 ? `
        <div style="display:flex;justify-content:space-between;margin-top:12px;">
          <span style="font-size:13px;font-weight:500;color:#000000;">Advance Payment</span>
          <span style="font-size:13px;font-weight:600;color:#000000;">${parseFloat(sale.paid_amount).toLocaleString(undefined,{minimumFractionDigits:2})}</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:15px;font-weight:700;color:#000000;border-top:1px solid #999999;padding-top:10px;margin-top:10px;">
    ...
    <!-- Footer -->
    <div style="margin-top:36px;padding-top:20px;border-top:1px solid #cccccc;">

      <!-- Terms & Conditions -->
      <div style="font-size:11.5px;color:#000000;line-height:1.7;margin-bottom:28px;">
        ${showRefundPolicy && refundPolicy ? `<div><span style="font-weight:700;color:#000000;">Terms &amp; Conditions:</span> ${refundPolicy}</div>` : ''}
        ${showFooterText && footerText ? `<div>${footerText}</div>` : (showFooterText ? '<div style="font-weight:600;color:#000000;">Thank you for your business!</div>' : '')}
      </div>

      <!-- Authorized Signature (left-aligned) -->
      <div style="display:inline-block;text-align:left;">
        <div style="width:180px;height:48px;border-bottom:1.5px solid #000000;"></div>
        <div style="margin-top:6px;font-size:10px;font-weight:700;color:#000000;">Authorized Signature</div>
        <div style="font-size:11px;color:#666666;margin-top:2px;">${businessName || ''}</div>
      </div>

    </div>

    <!-- Powered By: bottom-right -->
    <div style="margin-top:20px;padding-top:12px;border-top:1px solid #cccccc;display:flex;justify-content:flex-end;">
      <div style="font-size:10px;font-weight:600;color:#999999;">Generated by Inzeedo ERP System &bull; 2026</div>
    </div>
  </div>
    </div>
  </div>
  ...
  printDirect: async (sale, t) => {
    ...
    if (isManufacturing) {
      ...
      const itemRows = items.map((item, idx) =>
        `<tr>
          <td style="padding:8px 10px;border-bottom:1px solid #ddd;">${idx + 1}</td>
          <td style="padding:8px 10px;border-bottom:1px solid #ddd;font-weight:600;">${item.product_name || item.product?.name || item.name || 'Item'}</td>
          <td style="padding:8px 10px;border-bottom:1px solid #ddd;">${item.variant?.name || item.product_variant?.name || item.variant_name || '-'}</td>
...
<div style="display:flex;justify-content:space-between;border-bottom:1.5px solid #000;padding-bottom:14px;margin-bottom:14px;">
  <div>
    ...
    ${companyAddress ? `<div style="font-size:11px;color:#000;margin-top:3px;">${companyAddress}</div>` : ''}
    ${companyPhone ? `<div style="font-size:11px;color:#000;">Tel: ${companyPhone}</div>` : ''}
    ${taxId ? `<div style="font-size:11px;color:#000;">VAT/TIN: ${taxId}</div>` : ''}
  </div>
  <div style="text-align:right;">
    <div style="font-size:22px;font-weight:900;">Invoice</div>
    <div style="font-size:14px;font-weight:700;margin-top:8px;">${sale.invoice_number || 'DRAFT'}</div>
    <div style="font-size:11px;color:#000;margin-top:3px;">${invoiceDate}</div>
  </div>
</div>
<div style="margin-bottom:14px;">
  <div style="font-size:9px;font-weight:700;color:#000;margin-bottom:6px;">DISPATCHED TO</div>
  ${(() => { const dist = sale.distributor || sale.customer; return dist ? `
  <div style="font-size:14px;font-weight:800;">${dist.name || ''}</div>
  ${dist.company_name ? `<div style="font-size:11px;color:#000;margin-top:2px;">${dist.company_name}</div>` : ''}
  ${dist.phone ? `<div style="font-size:11px;color:#000;margin-top:2px;">Tel: ${dist.phone}</div>` : ''}
  ${dist.email ? `<div style="font-size:11px;color:#000;margin-top:2px;">${dist.email}</div>` : ''}
  ${dist.address ? `<div style="font-size:11px;color:#000;margin-top:2px;">${dist.address}</div>` : ''}` : '<div style="font-size:12px;font-weight:600;color:#000;">Walk-in / No Customer Selected</div>'; })()}
</div>
<table style="margin-bottom:14px;border:1px solid #000;">
...
<div style="display:flex;justify-content:flex-end;margin-bottom:30px;">
  <div style="width:250px;border:1px solid #000;padding:12px;">
...
<div style="border-top:1px solid #000;padding-top:12px;display:flex;justify-content:space-between;align-items:flex-end;">
  <div>
    ${showRefundPolicy && refundPolicy ? `<div style="font-size:10px;color:#000;"><strong>Terms:</strong> ${refundPolicy}</div>` : ''}
    ${showFooterText && footerText ? `<div style="font-size:10px;margin-top:3px;">${footerText}</div>` : (showFooterText ? '<div style="font-size:11px;font-weight:600;">Thank you for your business!</div>' : '')}
  ...
  <div style="font-size:9px;color:#000;">Generated by Inzeedo ERP System &bull; 2026</div>
</div>
</body></html>`;
    } else {
      const width = paperWidth === '58mm' ? '58mm' : '80mm';
