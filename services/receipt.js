...
<style>
  @page { size: A4; margin: 10mm; }
  * { box-sizing: border-box; margin: 0; padding: 0; }
  .invoice-body { font-family: system-ui, Helvetica, Arial, sans-serif; font-size: 12px; color: #000; background: #fff; line-height: 1.5; padding: 20px; max-width: 800px; margin: 0 auto; min-height: 100vh; display: flex; flex-direction: column; }
...
<img src="${qrUrl}" style="width:70px;height:70px;display:block;margin:10px auto 0;"/>
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        const receiptFilename = `Receipt-${sale.invoice_number || 'draft'}.pdf`;
        await generateAndSharePdf(htmlContent, receiptFilename, isManufacturing);
        return;
      }
    } catch (e) {
